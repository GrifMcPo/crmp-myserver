...
CMD:music(playerid) // уберите /music старый пж
{
    ...
    {
        new title[128];
        format(title, sizeof(title), "{FF5252}%s{ffffff} | Выберите нужное действие", NameServer[WEST_NAME]);
       ...
       Action(playerid, "Включил(-а) музыку в транспортном средстве", _, false);
        switch(listitem)
        {
            ...
            case 10:
            {
                ...
                SendClientMessage(playerid, -1, ""SC"Песня {FF5252}'Так похуй-Madk1d' {FFFFFF}включена.");
            }
            case 11:
            {
...
{
    new title[128];
    format(title, sizeof(title), "{FF5252}%s{ffffff} | Выберите нужную радиостанцию", NameServer[WEST_NAME]);
...
{
    new title[128];
    format(title, sizeof(title), "{FF5252}%s{ffffff} | Аудио-система автомобиля", NameServer[WEST_NAME]);
