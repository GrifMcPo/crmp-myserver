...
if (dialogid == 2222) {
            ...
            switch(listitem)
            {
                ...
                case 1:
                {
                    if(!GetFamily(f, family_status_storage) && GetPlayerFamily(playerid, RANG_FAMILY) < 5)
                        return ShowNotificationNew(playerid,  2, 6, 0, 0, "Сейф закрыт", "qq");
    ...
    if(dialogid == 2230)
    {
        if(GetPVarInt(playerid, "select_get_dostup"))
        {
            ...
            if(!IsPlayerConnected(to_player))
                return ShowNotificationNew(playerid,  2, 6, 0, 0, "Такого игрока нет", "qq");
    ...
    if(dialogid == 2237)
    {
        ...
        if(!type)
        {
            ...
            if(!c) return ShowNotificationNew(playerid,  2, 6, 0, 0, "Логи не найдены", "qq");
        ...
        if(listitem == up || listitem == down)
        {
            ...
            if(!rows) return ShowNotificationNew(playerid,  2, 6, 0, 0, "Логи не найдены", "qq");
        ...
        if(response)
        {
            if(GetPlayerDonateRub(playerid) >= 100)
            {
                ...
                if(!IsPlayerInRangeOfPlayer(playerid, to_player, 5.0))
                    return ShowNotificationNew(playerid,  2, 6, 0, 0, "Игрок слишком далеко", "qq");
...
CMD:famsklad(playerid)
{
    ...
    if(GetPlayerData(playerid, P_LAST_MP) == 1) return ShowNotificationNew(playerid,  2, 6, 0, 0, "Вы не можете использовать склад на мероприятии", "qq");
...
CMD:famcars(playerid)
{
    ...
    if(!rows)
        ShowNotificationNew(playerid,  2, 6, 0, 0, "У Вашей семьи нет транспорта", "qq");
